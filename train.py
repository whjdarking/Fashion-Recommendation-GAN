...
if(params['dataset'] == 'MNIST'):
    ...
    from models.mnist_model import Generator, Discriminator, DHead, QHead
elif(params['dataset'] == 'mydata'):
    from models.mydata_model import Generator, Discriminator, DHead, QHead
...
optimG = optim.Adam([{'params': netG.parameters()}, {'params': netQ.parameters()}], lr=1e-3, betas=(params['beta1'], params['beta2']))

#fixed_noise,_ = noise_sample(params['num_dis_c'], params['dis_c_dim'], params['num_con_c'], params['num_z'], 100, device=device)
#print(fixed_noise.size())
# Fixed Noise 缝合noise和有意义的discrete code
z = torch.randn(10*params['dis_c_dim'], params['num_z'], 1, 1, device=device)
fixed_noise = z
if(params['num_dis_c'] != 0):
    idx = np.arange(params['dis_c_dim']).repeat(10)  #
    dis_c = torch.zeros(10*params['dis_c_dim'], params['num_dis_c'], params['dis_c_dim'], device=device)
    for i in range(params['num_dis_c']):
        dis_c[torch.arange(0, 10*params['dis_c_dim']), i, idx] = 1.0

    dis_c = dis_c.view(10*params['dis_c_dim'], -1, 1, 1)

    fixed_noise = torch.cat((fixed_noise, dis_c), dim=1)

if(params['num_con_c'] != 0):
    con_c = torch.rand(10*params['dis_c_dim'], params['num_con_c'], 1, 1, device=device) * 2 - 1
...
#        print(fake_data.size())
        ...
        output = discriminator(fake_data) #1
        label.fill_(real_label)
        probs_fake = netD(output).view(-1) #2
